...
app.post('/api/login', (req, res) => {
    ...
    if (!login || !password) return res.status(400).json({ ok: false, mensaje: 'login y password son requeridos.' });
    ...
    conn.query(sql, [login], (err, results) => {
        ...
        bcrypt.compare(password, hashCompatible, (errBcrypt, passwordValido) => {
            if (errBcrypt || !passwordValido) return res.status(401).json({ ok: false, mensaje: 'Credenciales incorrectas.' });
...
app.post('/api/usuarios', verificarToken, (req, res) => {
    ...
    conn.query('SELECT id FROM user WHERE email = ? OR usuario = ?', [email, usuario], (err, existe) => {
        ...
        bcrypt.hash(password, 10, (errHash, hash) => {
            ...
            const sql = `INSERT INTO user (nombre, usuario, email, password, rolusuario_id, empresa_id,
            ...
            conn.query(sql, [nombre, usuario, email, hash, rolusuario_id, empresa_id], (err2, result) => {
                ...
                conn.query('INSERT INTO persona (usuario_id, empresa_id, estado_id) VALUES (?, ?, 1)', [nuevoId, empresa_id], (err3) => {
...
app.put('/api/usuarios/:id', verificarToken, (req, res) => {
    ...
    conn.query('SELECT id FROM user WHERE (email = ? OR usuario = ?) AND id != ?', [email, usuario, id], (err, existe) => {
        if (err) return res.status(500).json({ ok: false, mensaje: err.message });
        if (existe.length > 0) return res.status(400).json({ ok: false, mensaje: 'El email o usuario ya existe.' });
                ...
                bcrypt.hash(pass, 10, (errHash, hash) => {
                    ...
                    conn.query(sql, [nombre, usuario, email, hash, rolId, empId], (err2, result) => {
                        if (err2) {
                            ...
                            conn.query('INSERT INTO persona (usuario_id, empresa_id, estado_id) VALUES (?, ?, 1)', [result.insertId, empId]);
                            creados++;
                        }
                        procesados++;
                        if (procesados === total) res.json({ ok: true, creados, errores });
...
app.get('/api/faq', verificarToken, (req, res) => {
    const sql = `SELECT id, categoria, pregunta, respuesta, orden FROM faq WHERE estado_id = 1 ORDER BY categoria, orden`;
...
app.put('/api/perfil', verificarToken, (req, res) => {
    ...
    const sql = `UPDATE persona SET telefono=?, fecha_nacimiento=?, pais=?, ciudad=?,
                    direccion=?, tiempo_compania=?, tiempo_cargo=?,
                    linkedin=?, instagram=?, x_twitter=?, facebook=?,
                    pagina_web=?, observacion=?
                 WHERE usuario_id=?`;
    conn.query(sql, [telefono, fecha_nacimiento, pais, ciudad, direccion,
                     tiempo_compania, tiempo_cargo, linkedin, instagram,
                     x_twitter, facebook, pagina_web, observacion, id], (err) => {
        if (err) return res.status(500).json({ ok: false, mensaje: err.message });
...
app.put('/api/sesiones/:id', verificarToken, (req, res) => {
    const { id } = req.params;
    const { nombre_sesion, fecha_sesion, lugar, usuario_id } = req.body;
    conn.query('UPDATE sesion SET nombre_sesion=?, fecha_sesion=?, lugar=?, usuario_id=? WHERE id=?',
        [nombre_sesion, fecha_sesion, lugar, usuario_id, id], (err) => {
        if (err) return res.status(500).json({ ok: false, mensaje: err.message });
...
app.delete('/api/sesiones/:id', verificarToken, (req, res) => {
    conn.query('UPDATE sesion SET estado_id = 2 WHERE id = ?', [req.params.id], (err) => {
        if (err) return res.status(500).json({ ok: false, mensaje: err.message });
...
app.post('/api/sesiones/:id/activar', verificarToken, (req, res) => {
    conn.query('UPDATE sesion SET estado_id = 1 WHERE id = ?', [req.params.id], (err) => {
        if (err) return res.status(500).json({ ok: false, mensaje: err.message });
...
app.post('/api/sesiones/:id/finalizar', verificarToken, (req, res) => {
    conn.query('UPDATE sesion SET estado_id = 4 WHERE id = ?', [req.params.id], (err) => {
        if (err) return res.status(500).json({ ok: false, mensaje: err.message });
...
app.get('/api/sesiones/:id/detalle', verificarToken, (req, res) => {
    const { id } = req.params;
    conn.query('SELECT s.*, u.nombre as cliente FROM sesion s LEFT JOIN user u ON u.id = s.usuario_id WHERE s.id = ?', [id], (err, sesiones) => {
        if (err) return res.status(500).json({ ok: false, mensaje: err.message });
...
app.put('/api/sesiones/:id/detalle', verificarToken, (req, res) => {
    ...
    const actualizarTiempo = (cb) => {
        if (duracion !== undefined && segundos !== undefined) {
            conn.query('UPDATE sesion SET duracion=?, segundos=? WHERE id=?', [duracion, segundos, id], cb);
        } else { cb(null); }
    };

    actualizarTiempo((err) => {
        if (err) return res.status(500).json({ ok: false, mensaje: err.message });
        if (!actividades || Object.keys(actividades).length === 0) return res.json({ ok: true, mensaje: 'Tiempo actualizado.' });

        const entries = Object.entries(actividades);
        let procesados = 0;
        entries.forEach(([tipoId, texto]) => {
            conn.query('SELECT id FROM actividad WHERE sesion_id = ? AND tipo_contenido_id = ?', [id, tipoId], (err2, existe) => {
                if (err2) { procesados++; if (procesados === entries.length) res.json({ ok: true }); return; }
                if (existe.length > 0) {
                    conn.query('UPDATE actividad SET actividad=? WHERE sesion_id=? AND tipo_contenido_id=?', [texto, id, tipoId], () => {
                        procesados++; if (procesados === entries.length) res.json({ ok: true, mensaje: 'Actividades guardadas.' });
                    });
                } else {
                    conn.query('INSERT INTO actividad (actividad, sesion_id, tipo_contenido_id, estado_id) VALUES (?,?,?,1)', [texto, id, tipoId], () => {
                        procesados++; if (procesados === entries.length) res.json({ ok: true, mensaje: 'Actividades guardadas.' });
                    });
                }
            });
...
app.put('/api/sesiones/:id/finalizar-detalle', verificarToken, (req, res) => {
    const { id } = req.params;
    const { actividades, duracion, segundos } = req.body;
    conn.query('UPDATE sesion SET estado_id=4, duracion=?, segundos=? WHERE id=?', [duracion || 0, segundos || 0, id], (err) => {
        if (err) return res.status(500).json({ ok: false, mensaje: err.message });
        if (!actividades || Object.keys(actividades).length === 0) return res.json({ ok: true, mensaje: 'Sesión finalizada.' });
        ...
        entries.forEach(([tipoId, texto]) => {
            conn.query('SELECT id FROM actividad WHERE sesion_id = ? AND tipo_contenido_id = ?', [id, tipoId], (err2, existe) => {
                if (existe && existe.length > 0) {
                    conn.query('UPDATE actividad SET actividad=? WHERE sesion_id=? AND tipo_contenido_id=?', [texto, id, tipoId], () => {
                        procesados++; if (procesados === entries.length) res.json({ ok: true, mensaje: 'Sesión finalizada correctamente.' });
                    });
                } else {
                    conn.query('INSERT INTO actividad (actividad, sesion_id, tipo_contenido_id, estado_id) VALUES (?,?,?,1)', [texto, id, tipoId], () => {
                        procesados++; if (procesados === entries.length) res.json({ ok: true, mensaje: 'Sesión finalizada correctamente.' });
...
app.get('/api/herramientas/catalogo', verificarToken, (req, res) => {
    conn.query('SELECT id, nombre, descripcion, foto FROM herramienta WHERE estado_id = 1 ORDER BY nombre', (err, results) => {
        if (err) return res.status(500).json({ ok: false, mensaje: err.message });
        res.json({ ok: true, data: results });
    });
});
...
app.get('/api/herramientas/asignadas/:usuarioId', verificarToken, (req, res) => {
    const sql = `SELECT herramienta_id FROM herramienta_usuario WHERE usuario_id = ? AND estado_id = 1`;
    conn.query(sql, [req.params.usuarioId], (err, results) => {
        if (err) return res.status(500).json({ ok: false, mensaje: err.message });
...
app.delete('/api/herramientas/:id', verificarToken, (req, res) => {
    conn.query('UPDATE herramienta_usuario SET estado_id = 2 WHERE id = ?', [req.params.id], (err) => {
        if (err) return res.status(500).json({ ok: false, mensaje: err.message });
...
app.post('/api/herramientas/:id/activar', verificarToken, (req, res) => {
    conn.query('UPDATE herramienta_usuario SET estado_id = 1 WHERE id = ?', [req.params.id], (err) => {
        if (err) return res.status(500).json({ ok: false, mensaje: err.message });
...
app.get('/api/mis-herramientas/:id/cuadrantes', verificarToken, (req, res) => {
    const { id } = req.params;
    conn.query('SELECT hu.id, h.nombre, h.descripcion FROM herramienta_usuario hu LEFT JOIN herramienta h ON h.id = hu.herramienta_id WHERE hu.id = ? AND hu.usuario_id = ?',
        [id, req.usuario.id], (err, herr) => {
        if (err) return res.status(500).json({ ok: false, mensaje: err.message });
        ...
        const sqlEntidades = `SELECT e.id, e.entidad, e.tipo_entidad, e.sub_entidad, e.codigo_metodo
                              ...
                              WHERE he.herramienta_id = (SELECT herramienta_id FROM herramienta_usuario WHERE id = ?) AND he.estado_id = 1
                              ORDER BY e.id`;
        conn.query(sqlEntidades, [id], (err2, entidades) => {
            if (err2) return res.status(500).json({ ok: false, mensaje: err2.message });
            conn.query('SELECT * FROM atributo WHERE herramienta_usuario_id = ? AND estado_id = 1 ORDER BY fecha ASC', [id], (err3, atributos) => {
                if (err3) return res.status(500).json({ ok: false, mensaje: err3.message });
                const atributosMap = {};
                atributos.forEach(a => {
                ...
                res.json({ ok: true, herramienta: herr[0], entidades, atributos: atributosMap });
            });
        });
    });
});
...
app.post('/api/mis-herramientas/:id/atributo', verificarToken, (req, res) => {
    ...
    if (!atributo || !entidad_id) return res.status(400).json({ ok: false, mensaje: 'Datos incompletos.' });
    conn.query('INSERT INTO atributo (atributo, herramienta_usuario_id, entidad_id, estado_id, fecha) VALUES (?, ?, ?, 1, NOW())',
        [atributo, id, entidad_id], (err, result) => {
        if (err) return res.status(500).json({ ok: false, mensaje: err.message });
...
app.put('/api/mis-herramientas/atributo/:atributoId', verificarToken, (req, res) => {
    conn.query('UPDATE atributo SET atributo = ? WHERE id = ?', [req.body.atributo, req.params.atributoId], (err) => {
        if (err) return res.status(500).json({ ok: false, mensaje: err.message });
...
app.delete('/api/mis-herramientas/atributo/:atributoId', verificarToken, (req, res) => {
    conn.query('UPDATE atributo SET estado_id = 2 WHERE id = ?', [req.params.atributoId], (err) => {
        if (err) return res.status(500).json({ ok: false, mensaje: err.message });
...
app.get('/api/evaluaciones', verificarToken, (req, res) => {
    ...
    const sql = `SELECT eu.id, eu.intentos, eu.ver_resultados,
                 ...
                 WHERE eu.estado_id = ? AND eu.usuario_id IS NOT NULL AND eu.evaluacion_id IS NOT NULL
...
app.get('/api/evaluaciones/asignadas/:usuarioId', verificarToken, (req, res) => {
    conn.query('SELECT evaluacion_id FROM evaluacion_usuario WHERE usuario_id = ? AND estado_id = 1', [req.params.usuarioId], (err, results) => {
        if (err) return res.status(500).json({ ok: false, mensaje: err.message });
...
app.put('/api/evaluaciones/:id', verificarToken, (req, res) => {
    ...
    conn.query('UPDATE evaluacion_usuario SET intentos=?, ver_resultados=? WHERE id=?', [intentos, ver_resultados, req.params.id], (err) => {
        if (err) return res.status(500).json({ ok: false, mensaje: err.message });
...
app.delete('/api/evaluaciones/:id', verificarToken, (req, res) => {
    conn.query('UPDATE evaluacion_usuario SET estado_id = 2 WHERE id = ?', [req.params.id], (err) => {
        if (err) return res.status(500).json({ ok: false, mensaje: err.message });
...
app.post('/api/evaluaciones/:id/activar', verificarToken, (req, res) => {
    conn.query('UPDATE evaluacion_usuario SET estado_id = 1 WHERE id = ?', [req.params.id], (err) => {
        if (err) return res.status(500).json({ ok: false, mensaje: err.message });
...
app.get('/api/mis-evaluaciones', verificarToken, (req, res) => {
    const sql = `SELECT eu.id, eu.intentos, eu.ver_resultados,
                        eu.fecha, eu.inicio, eu.finalizacion, eu.estado_id,
                        e.id as evaluacion_id, e.nombre, e.descripcion, e.foto, e.instrucciones
                 FROM evaluacion_usuario eu
                 INNER JOIN evaluacion e ON e.id = eu.evaluacion_id AND e.estado_id = 1
...
app.get('/api/mis-evaluaciones/:id/test', verificarToken, (req, res) => {
    ...
    conn.query(sqlEval, [id, req.usuario.id], (err, evals) => {
        ...
        const ev = evals[0];
        conn.query('SELECT p.id, p.enunciado, p.numero, p.dimension_id FROM preguntas p WHERE p.evaluacion_id = ? AND p.estado_id = 1 ORDER BY p.numero ASC',
            [ev.evaluacion_id], (err2, preguntas) => {
            if (err2) return res.status(500).json({ ok: false, mensaje: err2.message });

            conn.query('SELECT id, alternativa, score, letra, cuadrante, grupo, pregunta_id FROM alternativas WHERE evaluacion_id = ? AND estado_id = 1 ORDER BY pregunta_id, id ASC',
                [ev.evaluacion_id], (err3, alternativas) => {
                if (err3) return res.status(500).json({ ok: false, mensaje: err3.message });

                const sqlResp = `SELECT dr.alternativa_id, dr.pregunta_id, dr.resultado
                                 FROM respuestas r
                ...
                conn.query(sqlResp, [id], (err4, respuestas) => {
                    ...
                    if (esDisc) {
                        // Para DISC: { preguntaId: { mas: altId, menos: altId } }
                        respuestas.forEach((r) => {
                            if (!respuestasMap[r.pregunta_id]) respuestasMap[r.pregunta_id] = { mas: null, menos: null };
                            if (String(r.resultado) === '1')  respuestasMap[r.pregunta_id].mas   = r.alternativa_id;
                            if (String(r.resultado) === '-1') respuestasMap[r.pregunta_id].menos = r.alternativa_id;
                        });
                    } else {
                        respuestas.forEach((r) => { respuestasMap[r.pregunta_id] = r.alternativa_id; });
                    }
...
app.post('/api/mis-evaluaciones/:id/guardar', verificarToken, (req, res) => {
    ...
    conn.query(sqlEval, [id, req.usuario.id], (err, evals) => {
        ...
        if (ev.estado_id === 4) return res.status(400).json({ ok: false, mensaje: 'Esta evaluación ya fue finalizada.' });

        // Marcar inicio si es primera vez
        conn.query('UPDATE evaluacion_usuario SET inicio = COALESCE(inicio, NOW()), estado_id = 3 WHERE id = ? AND estado_id = 1', [id], () => {});

        // Obtener o crear registro en respuestas
        const crearOUsarRespuesta = (cb) => {
            conn.query('SELECT id FROM respuestas WHERE evaluacion_usuario_id = ? LIMIT 1', [id], (err, rows) => {
                ...
                conn.query('INSERT INTO respuestas (evaluacion_usuario_id, estado_id, fecha, nro_intentos) VALUES (?, 1, NOW(), 1)', [id], (err2, result) => {
                    if (err2) return res.status(500).json({ ok: false, mensaje: err2.message });
                    cb(result.insertId);
                });
            });
        ...
        crearOUsarRespuesta((respuestaId) => {
            ...
            const finalizarEval = (calcular) => {
                conn.query('UPDATE evaluacion_usuario SET estado_id = 4, finalizacion = NOW() WHERE id = ?', [id], (err) => {
                    if (err) return res.status(500).json({ ok: false, mensaje: err.message });
                    if (calcular) calcular();
                    else res.json({ ok: true, mensaje: 'Evaluación finalizada correctamente.' });
                });
            };
            ...
            if (esDisc) {
                ...
                const siguiente = () => {
                    ...
                    const guardarAlt = (altId, resultado, next) => {
                        ...
                        conn.query('SELECT id FROM detalle_respuesta WHERE respuesta_id = ? AND pregunta_id = ? AND resultado = ? LIMIT 1',
                            [respuestaId, pregId, resultado], (err, rows) => {
                            if (err) return res.status(500).json({ ok: false, mensaje: err.message });
                            if (rows.length > 0) {
                                conn.query('UPDATE detalle_respuesta SET alternativa_id = ? WHERE id = ?', [altId, rows[0].id], next);
                            } else {
                                conn.query('INSERT INTO detalle_respuesta (respuesta_id, alternativa_id, pregunta_id, resultado, estado_id) VALUES (?,?,?,?,1)',
                ...
                // ── Tests normales: upsert con score correcto ─────────────────
                const entries = Object.entries(respuestas || {});
                if (entries.length === 0) {
                ...
                const altIds = entries.map(([, altId]) => Number(altId));
                const pregIds = entries.map(([pregId]) => Number(pregId));

                conn.query('SELECT id, score, cuadrante, grupo, letra FROM alternativas WHERE id IN (?)', [altIds], (err, altsData) => {
                    if (err) return res.status(500).json({ ok: false, mensaje: err.message });

                    conn.query('SELECT id, inversa FROM preguntas WHERE id IN (?)', [pregIds], (err2, pregsData) => {
                        if (err2) return res.status(500).json({ ok: false, mensaje: err2.message });
                        ...
                        const siguiente = () => {
                            if (idx >= entries.length) {
                                if (finalizar) {
                                    return finalizarEval(() => {
                                        calcularResultados(ev.evaluacion_id, id, respuestaId, altIds, altsData, res);
                                    });
                                }
                                return res.json({ ok: true, mensaje: 'Avance guardado.' });
                            }
                            ...
                            const alt  = altsData.find(a => a.id === altIdNum);
                            const preg = pregsData.find(p => p.id === Number(pregId));

                            let score = alt ? (alt.score || 0) : 0;

                            // Ajuste de score según tipo de test
                            if (ev.evaluacion_id === 6) {
                                score = alt ? (alt.cuadrante || 0) : 0;
                            } else if (ev.evaluacion_id === 5 && preg && preg.inversa == 0) {
                                score = 7 - score;
                            } else if (ev.evaluacion_id === 4 && preg && preg.inversa == 0) {
                                score = score == 1 ? 0 : 1;
                            }

                            conn.query('SELECT id FROM detalle_respuesta WHERE respuesta_id = ? AND pregunta_id = ? LIMIT 1',
                                [respuestaId, pregId], (err3, rows) => {
                                if (err3) return res.status(500).json({ ok: false, mensaje: err3.message });
                                if (rows.length > 0) {
                                    conn.query('UPDATE detalle_respuesta SET alternativa_id=?, resultado=? WHERE id=?',
                                        [altIdNum, score, rows[0].id], siguiente);
                                } else {
                                    conn.query('INSERT INTO detalle_respuesta (respuesta_id, alternativa_id, pregunta_id, resultado, estado_id) VALUES (?,?,?,?,1)',
                                        [respuestaId, altIdNum, pregId, score], siguiente);
                                }
                            });
                        };
                        siguiente();
...
app.get('/api/mis-evaluaciones/:id/resultados', verificarToken, (req, res) => {
    ...
    const sql = `SELECT eu.id, eu.resultados, eu.finalizacion, eu.estado_id,
                        e.id as evaluacion_id, e.nombre, e.descripcion, e.interpretacion, e.foto
                 FROM evaluacion_usuario eu
                 INNER JOIN evaluacion e ON e.id = eu.evaluacion_id
                 WHERE eu.id = ? AND eu.usuario_id = ?`;
...
function calcularResultados(idTest, euId, respuestaId, altIds, altsData, res) {
    const guardarResultados = (jsonResultados) => {
        conn.query('UPDATE evaluacion_usuario SET resultados=? WHERE id=?', [JSON.stringify(jsonResultados), euId], (err) => {
            if (err) return res.status(500).json({ ok: false, mensaje: err.message });
            res.json({ ok: true, mensaje: 'Evaluación finalizada correctamente.' });
        });
    };

    // IDs 2, 5: Tests globales
    if ([2, 5].includes(idTest)) {
        ...
        const puntajeFinal = idTest === 5
            ...
            : puntajeTotal;
        conn.query('SELECT * FROM resultados WHERE evaluacion_id=? AND minimo<=? AND maximo>=? LIMIT 1', [idTest, puntajeFinal, puntajeFinal], (err, diag) => {
            if (err) return res.status(500).json({ ok: false, mensaje: err.message });
            const d = diag[0];
            ...
            conn.query('SELECT * FROM dimension WHERE id IN (?)', [dimIds], (err2, dimensiones) => {
                ...
                dims.forEach(row => {
                    const puntaje = idTest === 9
                        ...
                        : Number(row.sumaTotal);
                    const dimObj   = dimensiones.find(d => d.id === row.dimId);
                    const nombreDim = dimObj ? dimObj.dimension : 'Dimensión';
                    const descDim   = idTest === 3 ? (dimObj ? dimObj.descripcion_larga : '') : (dimObj ? dimObj.descripcion : '');
                    ...
                    const paramsDiag = idTest === 3 ? [idTest, puntaje, puntaje] : [idTest, row.dimId, puntaje, puntaje];
    ...
    // ID 1: Liderazgo Situacional
        ...
        conn.query('SELECT * FROM resultados WHERE evaluacion_id=? AND minimo<=? AND maximo>=? LIMIT 1', [idTest, nivelM, nivelM], (err, diag) => {
            const d = diag ? diag[0] : null;
            guardarResultados({ Resultados: [
    ...
    // ID 6: Manejo de Conflictos
        ...
        conn.query('SELECT * FROM dimension WHERE id IN (?)', [dimIds], (err, dims) => {
            ...
            Object.entries(mapeoCuadrantes).forEach(([cuadrante, dimId]) => {
                ...
                conn.query('SELECT * FROM resultados WHERE evaluacion_id=? AND dimension_id=? AND minimo<=? AND maximo>=? LIMIT 1', [idTest, dimId, puntaje, puntaje], (err2, diag) => {
                    const d = diag ? diag[0] : null;
    ...
    // ID 7: Estilos de Aprendizaje
        ...
        conn.query(
            ...
            [respuestaId], (err, cuads) => {
            if (err) return res.status(500).json({ ok: false, mensaje: err.message });
            const get = (c) => { const r = cuads.find(x => x.cuadrante == c); return r ? Number(r.total) : 0; };
            ...
            conn.query('SELECT * FROM dimension WHERE id IN (?)', [dimIds], (err2, dims) => {
                conn.query('SELECT * FROM resultados WHERE evaluacion_id=? AND resultado=? LIMIT 1', [idTest, nombreCuadrante], (err3, diagCuad) => {
...
app.get('/api/instituciones', verificarToken, (req, res) => {
    const sql = `SELECT em.id, em.empresa, em.razonsocial, em.numero_identificacion_fiscal,
                 ...
                 WHERE em.estado_id = ?
                 ORDER BY em.id DESC`;
    conn.query(sql, [req.query.estado || 1], (err, rows) => {
...
app.post('/api/instituciones', verificarToken, (req, res) => {
    ...
    conn.query('INSERT INTO empresa (empresa, razonsocial, numero_identificacion_fiscal, direccion, telefonos, pagina_web, estado_id, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, 1, NOW(), NOW())',
        [empresa, razonsocial, numero_identificacion_fiscal, direccion, telefonos, pagina_web], (err, result) => {
...
app.put('/api/instituciones/:id', verificarToken, (req, res) => {
    const { empresa, razonsocial, numero_identificacion_fiscal, direccion, telefonos, pagina_web } = req.body;
    if (!empresa) return res.status(400).json({ ok: false, mensaje: 'El nombre es obligatorio.' });
    conn.query('UPDATE empresa SET empresa=?, razonsocial=?, numero_identificacion_fiscal=?, direccion=?, telefonos=?, pagina_web=?, updated_at=NOW() WHERE id=?',
        [empresa, razonsocial, numero_identificacion_fiscal, direccion, telefonos, pagina_web, req.params.id], (err) => {
        if (err) return res.status(500).json({ ok: false });
        res.json({ ok: true });
    });
});
...
app.post('/api/instituciones/carga-masiva', verificarToken, upload.single('archivo'), (req, res) => {
    ...
    try {
        ...
        let created = 0, updated = 0, skipped = 0, pending = rows.length;
        const done = () => { if (--pending === 0) res.json({ ok: true, created, updated, skipped }); };
        ...
        rows.forEach(row => {
            ...
            conn.query('SELECT id FROM empresa WHERE LOWER(empresa) = LOWER(?) LIMIT 1', [empresa], (err, found) => {
                if (err) { skipped++; done(); return; }
                if (found.length > 0) {
                    conn.query('UPDATE empresa SET razonsocial=?, numero_identificacion_fiscal=?, direccion=?, telefonos=?, pagina_web=?, updated_at=NOW() WHERE id=?',
                        [payload.razonsocial, payload.numero_identificacion_fiscal, payload.direccion, payload.telefonos, payload.pagina_web, found[0].id],
                        () => { updated++; done(); });
                } else {
                    conn.query('INSERT INTO empresa (empresa, razonsocial, numero_identificacion_fiscal, direccion, telefonos, pagina_web, estado_id, created_at, updated_at) VALUES (?,?,?,?,?,?,1,NOW(),NOW())',
                        [payload.empresa, payload.razonsocial, payload.numero_identificacion_fiscal, payload.direccion, payload.telefonos, payload.pagina_web],
                        () => { created++; done(); });
                }
            });
        });
...
app.get('/api/dashboard', verificarToken, (req, res) => {
    ...
    conn.query('SELECT COUNT(*) as total FROM user WHERE rolusuario_id = 3 AND estado_id = 1',
        (err, r) => { resultado.clientes_total = err ? 0 : r[0].total; check(); });
    conn.query('SELECT COUNT(DISTINCT usuario_id) as total FROM sesion WHERE estado_id = 1',
        (err, r) => { resultado.clientes_por_asesorar = err ? 0 : r[0].total; check(); });
    conn.query('SELECT COUNT(DISTINCT usuario_id) as total FROM sesion WHERE estado_id = 4',
        (err, r) => { resultado.clientes_asesorados = err ? 0 : r[0].total; check(); });
    conn.query('SELECT COUNT(*) as total FROM evaluacion_usuario WHERE estado_id IN (1, 3)',
        (err, r) => { resultado.evaluaciones_pendientes = err ? 0 : r[0].total; check(); });
    conn.query('SELECT COUNT(*) as total FROM evaluacion_usuario WHERE estado_id = 4',
        (err, r) => { resultado.evaluaciones_finalizadas = err ? 0 : r[0].total; check(); });
});

app.get('/api/dashboard/cliente', verificarToken, (req, res) => {
    ...
    conn.query('SELECT COUNT(*) as total FROM evaluacion_usuario WHERE usuario_id = ? AND estado_id IN (1, 3)',
        [usuarioId], (err, r) => { resultado.evaluaciones_pendientes = err ? 0 : r[0].total; check(); });
    conn.query('SELECT COUNT(*) as total FROM evaluacion_usuario WHERE usuario_id = ? AND estado_id = 4',
        [usuarioId], (err, r) => { resultado.evaluaciones_finalizadas = err ? 0 : r[0].total; check(); });
    conn.query('SELECT COUNT(*) as total FROM herramienta_usuario WHERE usuario_id = ? AND estado_id = 1',
        [usuarioId], (err, r) => { resultado.herramientas_asignadas = err ? 0 : r[0].total; check(); });
    conn.query('SELECT COUNT(*) as total FROM sesion WHERE usuario_id = ? AND estado_id = 1',
        [usuarioId], (err, r) => { resultado.sesiones_activas = err ? 0 : r[0].total; check(); });
});
...
app.get('/api/biblioteca', verificarToken, (req, res) => {
    ...
    const sql = `SELECT b.id, b.titulo, b.descripcion, b.tipo,
                        ...
                        u.nombre as usuario, e.nombre as estado
                 FROM biblioteca b
...
app.put('/api/biblioteca/digital/:id', verificarToken, upload.single('archivo'), (req, res) => {
    ...
    if (!titulo || !categoria_id) return res.status(400).json({ ok: false, mensaje: 'Título y categoría son requeridos.' });
...
app.put('/api/biblioteca/audiovisual/:id', verificarToken, (req, res) => {
    ...
    conn.query(`UPDATE biblioteca SET titulo=?, descripcion=?, url=?, categoria_id=?, updated_at=NOW() WHERE id=? AND tipo='audiovisual'`,
        [titulo, descripcion || null, url, categoria_id, req.params.id], (err) => {
        if (err) return res.status(500).json({ ok: false, mensaje: err.message });
...
app.get('/api/mi-biblioteca', verificarToken, (req, res) => {
    conn.query(`SELECT b.id, b.titulo, b.descripcion, b.tipo, b.ruta_archivo, b.url, c.nombre as categoria
